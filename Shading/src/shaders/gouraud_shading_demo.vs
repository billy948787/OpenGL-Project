...
vec3 Specular(vec3 Ks, vec3 I, vec3 N, vec3 lightDir, vec3 viewDir)
{
    
    return Ks * I * pow(max(0, dot(reflect(-lightDir, N), viewDir)), Ns);
}
...
void main()
{
    ...
    // Specular.
    vec3 specular = Specular(Ks, dirLightRadiance, vsNormal, vsLightDir, normalize(-vsPosition));
    ...
    // Specular.
     specular = Specular(Ks, radiance, vsNormal, vsLightDir, normalize(-vsPosition));
    vec3 pointLight = diffuse + specular;
    // -------------------------------------------------------------
    
    iLightingColor = specular;
}