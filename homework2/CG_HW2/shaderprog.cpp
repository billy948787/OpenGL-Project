...
bool ShaderProg::LoadFromFiles(const std::string vsFilePath,
  ...
  if (!LoadShaderTextFromFile(fsFilePath, fs)) {
    std::cerr << "[ERROR] Failed to load vertex shader source: " << fsFilePath
              << std::endl;
    return false;
  };
...
void PhongShadingDemoShaderProg::GetUniformVariableLocation() {
  ...
  locSpotLightIntensity =
      glGetUniformLocation(shaderProgId, "spotLightIntensity");
  ...
  locPointLightIntensity =
      glGetUniformLocation(shaderProgId, "pointLightIntensity");
  locSpotLightCutOffStart =
      glGetUniformLocation(shaderProgId, "spotLightCutOffStart");
  locSpotLightTotalWidth =
      glGetUniformLocation(shaderProgId, "spotLightTotalWidth");
  locIsBlingPhong = glGetUniformLocation(shaderProgId, "isBlingPhong");
  locOnPointLight = glGetUniformLocation(shaderProgId, "onPointLight");
  locOnSpotLight = glGetUniformLocation(shaderProgId, "onSpotLight");
  locOnDirLight = glGetUniformLocation(shaderProgId, "onDirLight");
