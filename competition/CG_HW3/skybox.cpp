...
Skybox::Skybox(const std::string& texImagePath, const int nSlices,
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(VertexPT) * vertices.size(),
...
void Skybox::Render(Camera* camera, SkyboxShaderProg* shader) {
  ...
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(VertexPT),
                        (const GLvoid*)12);
  ...
  glm::mat4x4 R = glm::rotate(glm::mat4x4(1.0f), glm::radians(rotationY),
...
void Skybox::CreateSphere3D(const int nSlices, const int nStacks,
                            const float radius, std::vector<VertexPT>& vertices,
  ...
  for (int t = 0; t <= numTheta; ++t) {
    for (int p = 0; p <= numPhi; ++p) {
      ...
      // std::cout << glm::degrees<float>(phi) << " " <<
      // glm::degrees<float>(theta) << std::endl;
      glm::vec2 uv =
          glm::vec2((float)p / (float)numPhi, (float)t / (float)numTheta);
